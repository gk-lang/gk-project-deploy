<template>
  <div class="hiddenFigure auto-height-container">
    <svg
      ref="svgRef"
      id="svgRef"
...
function generateShapeData() {
  ...
  if (targetPositionType === "Random") {
    ...
    const i = randomNum(0, 3);
    // config.value.targetPosition = list[i];
    config.value.targetPosition = list[1];
  }
...
function generateSquareData() {
  ...
  // 颗粒之间的间距
  const shapeSpacing = config.value.shapeSpacing - 5;
  // 边线的长度
  ...
  if (targetPosition === "first") {
    p.x = randomNum(
      containerWidth.value / 2 + marginSize,
  ...
  if (targetPosition === "third") {
    p.x = randomNum(0 + marginSize, containerWidth.value / 2 - lineLength);
    p.y = randomNum(
      containerHeight.value / 2 + marginSize,
...
function generateTriangleData() {
  ...
  const offsetWidth = radius + marginSize + shapeSize + shapeSpacing;
  //   if (targetPosition === "random") {
  //     p = {
...
function generateRectDom(x, y, angle, wrapName) {
  ...
  // 创建<use>元素
  const rectEl = document.createElementNS("http://www.w3.org/2000/svg", "use");
  // 设置属性
  rectEl.setAttributeNS("http://www.w3.org/1999/xlink", "xlink:href", "#shape");
  ...
  rectEl.setAttribute("x", x + shapeSize / 2);
  rectEl.setAttribute("y", y + shapeSize / 2);
  rectEl.setAttribute(
    "transform",
...
function fillScreenWithRects() {
  ...
  while (i < disruptorNum && f < maxIntersectNum) {
    const randomX = randomNum(marginSize, containerWidth.value - marginSize);
    const randomY = randomNum(marginSize, containerHeight.value - marginSize);
